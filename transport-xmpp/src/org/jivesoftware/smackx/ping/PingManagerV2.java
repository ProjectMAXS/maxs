...
public class PingManagerV2 {
	...
	private static final Map<Connection, PingManagerV2> instances = Collections
			.synchronizedMap(new WeakHashMap<Connection, PingManagerV2>());
	...
	public static synchronized PingManagerV2 getInstanceFor(Connection connection) {
		PingManagerV2 pingManager = instances.get(connection);

		if (pingManager == null) {
			pingManager = new PingManagerV2(connection);
		}
	...
	public IQ ping(String jid, long pingTimeout) {
		...
		PacketCollector collector = connection.createPacketCollector(new PacketIDFilter(ping
				.getPacketID()));
	...
	public boolean isPingSupported(String jid) {
		Connection connection = weakRefConnection.get();
		try {
			DiscoverInfo result = ServiceDiscoveryManager.getInstanceFor(connection).discoverInfo(
					jid);
	...
	protected synchronized void maybeSchedulePingServerTask() {
		...
		if (pingInterval > 0) {
			...
			periodicPingTask = periodicPingExecutorService.schedule(new ServerPingTask(
					weakRefConnection), pingInterval, TimeUnit.SECONDS);
		}
	}
	...
	static class ServerPingTask implements Runnable {
		...
		protected ServerPingTask(WeakReference<Connection> connection) {
